document.addEventListener('DOMContentLoaded', () => {
    ...
    function crearTarjeta(pelicula) {
        ...
        card.dataset.referencia = pelicula.referencia;
    ...
    function actualizarTarjeta(card, descripcion, referencia, tipo   ) {
        ...
        if (!isExpanded) {
            ...
            button.addEventListener('click', () => {
                ...
                if (tipo=="Serie") {
                    localStorage.setItem('idSerie', referencia);
                    localStorage.removeItem('idpelicula');
                } else if (tipo=="Pelicula"
                ) {
                    localStorage.setItem('idpelicula', referencia);
    ...
    function getMoviesFromAPI() {
        return axios
            .get('http://localhost:3003/populares2')
            .then((response) => {
    ...
    getMoviesFromAPI()
        .then((popularesDesdeAPI) => {
            popularesDesdeAPI.forEach((populares, index) => {
                const card = crearTarjeta(populares);
                cardsContainer.appendChild(card);

                actualizarTarjeta(card, populares.descripcion, populares.referencia, populares.tipo);
            });
        })
        .catch((error) => {
            console.error('Error al obtener los datos de películas desde la API:');
        });
